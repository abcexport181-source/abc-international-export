...
export async function GET(request: Request) {
  ...
  const by = searchParams.get('by') || 'path'; // dimension to group by
  const limit = searchParams.get('limit') || '10';

  // Build the API URL
  const apiUrl = new URL(`${VERCEL_API_BASE}/v1/query/web-analytics/visits/aggregate`);
